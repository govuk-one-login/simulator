...
export const getSigningKeyFromJwksUrl = async (
  ...
  if (!result.ok) {
    throw new Error(`Failed to fetch JWKS from URL ${url}`);
  }
...
export const getSigningKeyFromJwksList = (
  ...
  if (validKeys.length > 0) {
    ...
    throw new NoSigningKeyFound(url);
  }
};
export class NoSigningKeyFound extends Error {
  constructor(jwksUrl?: string) {
    super(`No RSA signing key found on JWKS URL ${jwksUrl || ""}`);
  }
}
