...
| Public Key Source               | How the public key used to validate the client_assertion signature should be retrieved. Either using a static key setup by the simulator, or a JWKS endpoint. Note that JWKS only works if you also set the `JWKS_URL` field.                                                                                                                               | `PUBLIC_KEY_SOURCE`               | publicKeySource                                | Either STATIC or JWKS                                                                                                                                                                                                                                                                            |
